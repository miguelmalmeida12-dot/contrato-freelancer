  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
            <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 20, background: tipo.bg, color: tipo.text }}>
              {tipo.label}
            </span>
            <span style={{ fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 20, background: status.bg, color: status.text }}>
              {status.label}
            </span>
          </div>
          {/* TÍTULO */}
          <div style={{ fontSize: 14, fontWeight: 600, color: '#1A1612', lineHeight: 1.3, marginBottom: 4, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
            {contrato.titulo}
          </div>
          {contrato.descricao && (
            <div style={{ fontSize: 12, color: '#A09890', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
              {contrato.descricao}
            </div>
          )}
        </div>
        ...
        <div style={{ position: 'relative', marginLeft: 8, flexShrink: 0 }}>
          <button
            ...
            style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '4px 8px', color: '#A09890', fontSize: 18, lineHeight: 1 }}
          >⋯</button>
          {menuOpen && (
            <>
              ...
              <div style={{ position: 'absolute', right: 0, top: '100%', background: '#fff', border: '1px solid #E2DDD6', borderRadius: 6, boxShadow: '0 8px 24px rgba(0,0,0,.12)', zIndex: 20, minWidth: 180, overflow: 'hidden' }}>
                <button onClick={() => { onAbrir(contrato); setMenuOpen(false) }}
                  style={{ display: 'block', width: '100%', textAlign: 'left', padding: '10px 16px', fontSize: 13, color: '#1A1612', background: 'none', border: 'none', cursor: 'pointer', borderBottom: '1px solid #F3F0EB' }}>
                  ✏️ Abrir / Editar
                </button>
                {contrato.status === 'rascunho' ? (
                  <button onClick={() => { onStatus(contrato.id, 'assinado'); setMenuOpen(false) }}
                    style={{ display: 'block', width: '100%', textAlign: 'left', padding: '10px 16px', fontSize: 13, color: '#065F46', background: 'none', border: 'none', cursor: 'pointer', borderBottom: '1px solid #F3F0EB' }}>
                    ✅ Marcar como assinado
                  </button>
                ) : (
                  <button onClick={() => { onStatus(contrato.id, 'rascunho'); setMenuOpen(false) }}
                    style={{ display: 'block', width: '100%', textAlign: 'left', padding: '10px 16px', fontSize: 13, color: '#6B7280', background: 'none', border: 'none', cursor: 'pointer', borderBottom: '1px solid #F3F0EB' }}>
                    ↩ Voltar para rascunho
                  </button>
                )}
                <button onClick={() => { if (confirm('Remover este contrato?')) { onRemover(contrato.id); setMenuOpen(false) } }}
                  style={{ display: 'block', width: '100%', textAlign: 'left', padding: '10px 16px', fontSize: 13, color: '#C8502A', background: 'none', border: 'none', cursor: 'pointer' }}>
                  🗑 Remover
                </button>
              </div>
            </>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 12, paddingTop: 10, borderTop: '1px solid #F3F0EB' }}>
        ...
        <button
          onClick={() => onAbrir(contrato)}
          style={{ fontSize: 12, fontWeight: 600, color: '#C8502A', background: 'none', border: 'none', cursor: 'pointer', padding: '4px 8px' }}
        >
          Baixar PDF →
        </button>
      </div>
...
export default function Perfil() {
  ...
  const handleAbrir = useCallback((contrato) => {
    // Salva o contrato no sessionStorage para o Gerador carregar
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F7F5F0' }}>
      ...
      <nav style={{
        ...
        <span
          onClick={() => navigate('/')}
          style={{ fontFamily: 'Georgia, serif', fontSize: 20, color: '#F7F5F0', cursor: 'pointer', letterSpacing: '-.02em' }}
        >
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: isMobile ? '20px 16px' : '32px 24px' }}>
        ...
        {/* HEADER DO PERFIL */}
        <div style={{
          background: '#fff', border: '1px solid #E2DDD6', borderRadius: 8,
          ...
          <div style={{
            ...
            background: '#1A1612', color: '#F7F5F0',
        ...
        <div style={{ display: 'flex', gap: 2, marginBottom: 20, background: '#E2DDD6', borderRadius: 6, padding: 3 }}>
          {[
            ...
            <button
              ...
              onClick={() => setAba(a.id)}
              style={{
                flex: 1, padding: isMobile ? '9px 8px' : '9px 16px',
                borderRadius: 4, border: 'none', cursor: 'pointer',
                fontSize: isMobile ? 12 : 13, fontWeight: 600,
                background: aba === a.id ? '#fff' : 'transparent',
                color: aba === a.id ? '#1A1612' : '#7A7268',
                transition: 'all .15s',
                boxShadow: aba === a.id ? '0 1px 4px rgba(0,0,0,.08)' : 'none',
              }}
            >
              {a.label}
            </button>
            ...
            <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
              <input
                ...
                style={{
                  ...
                  border: '1px solid #CEC8BF', borderRadius: 4,
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                {['todos', 'rascunho', 'assinado'].map(f => (
                  <button
                    ...
                    onClick={() => setFiltro(f)}
                    style={{
                      padding: '8px 14px', borderRadius: 4, border: '1px solid #CEC8BF',
                      fontSize: 12, fontWeight: 600, cursor: 'pointer',
                      background: filtro === f ? '#1A1612' : '#fff',
                      color: filtro === f ? '#F7F5F0' : '#5C5448',
                    }}
                  >
            ...
            {contratosFiltrados.length === 0 ? (
              <div style={{ background: '#fff', border: '1px solid #E2DDD6', borderRadius: 8, padding: '48px 24px', textAlign: 'center' }}>
                ...
                <div style={{ fontSize: 13, color: '#A09890', marginBottom: 24, lineHeight: 1.6 }}>
                  {contratos.length === 0
                    ? 'Seus contratos gerados ficarão salvos aqui para você baixar ou editar quando quiser.'
                    : 'Tente ajustar o filtro ou a busca.'}
                </div>
                {contratos.length === 0 && (
                  <button
                    onClick={() => navigate('/app')}
                    style={{ padding: '11px 24px', background: '#1A1612', color: '#F7F5F0', border: 'none', borderRadius: 4, fontSize: 14, fontWeight: 600, cursor: 'pointer' }}
                  >
        ...
        {aba === 'conta' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div style={{ background: '#FEF3ED', border: '1px solid #FDBA74', borderRadius: 8, padding: '16px 20px' }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: '#9A3412', marginBottom: 8 }}>⚠ Exportar e fazer backup</div>
              <div style={{ fontSize: 13, color: '#C2410C', lineHeight: 1.6, marginBottom: 12 }}>
                Seus contratos são salvos neste navegador. Para não perder o histórico, exporte uma cópia agora.
              </div>
              <button
                onClick={() => {
                  ...
                  a.href = url
                  a.download = `TRACT_backup_${new Date().toISOString().slice(0,10)}.json`
                  a.click()
                  URL.revokeObjectURL(url)
                }}
                style={{ padding: '9px 18px', background: '#C8502A', color: '#fff', border: 'none', borderRadius: 4, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}
              >
